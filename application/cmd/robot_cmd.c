...
#endif // GIMBAL_BOARD
    gimbal_cmd_send.pitch = PITCH_MID_DEG; // 上电初始化到劣弧中点
...
{
    static uint8_t last_switch_left = 0;

    // 控制云台运行模式,模式切换，云台回正
    if (switch_is_down(rc_data[TEMP].rc.switch_left)) // 左侧开关状态[下]，自稳模式
    {
        if (last_switch_left != 1) {
            gimbal_cmd_send.pitch = PITCH_MID_DEG; // 切换到劣弧中点
            gimbal_cmd_send.yaw = 17;
            last_switch_left = 1;
        }
        //chassis_cmd_send.chassis_mode = CHASSIS_ROTATE;
        gimbal_cmd_send.gimbal_mode = GIMBAL_GYRO_MODE;
    }
    else if (switch_is_mid(rc_data[TEMP].rc.switch_left)) // 左侧开关状态[中],编码器值模式（普通）
    {
        if (last_switch_left != 2) {
            gimbal_cmd_send.pitch = PITCH_MID_DEG; // 切换到劣弧中点
            gimbal_cmd_send.yaw = 17;
            last_switch_left = 2;
        }
    ...
    else if (switch_is_up(rc_data[TEMP].rc.switch_left)) //左侧开关状态[上]，无力状态
    {
        if (last_switch_left != 3) {
            gimbal_cmd_send.pitch = PITCH_MID_DEG; // 切换到劣弧中点
            gimbal_cmd_send.yaw = 17;
            last_switch_left = 3;
        }
    ...
    if (gimbal_cmd_send.gimbal_mode != GIMBAL_ZERO_FORCE)
    {
        ...
        if (rc_data[TEMP].rc.rocker_l_ > 10||rc_data[TEMP].rc.rocker_l_ < -10)
            ...
            gimbal_cmd_send.yaw = gimbal_fetch_data.gimbal_imu_data.YawTotalAngle; // 回中时锁定当前角度

        if (abs(rc_data[TEMP].rc.rocker_l1) > 5)
        {
        ...
        // pitch限制（弧度）：min=-0.8（最大俯）, max=0.14（最大仰）
        LIMIT_MIN_MAX(gimbal_cmd_send.pitch, pitch_limit_min, pitch_limit_max);
   ...
   // chassis_cmd_send.vx = 10.0f * (float)rc_data[TEMP].rc.rocker_r_; // _水平方向
    ...
    // 发射参数
    if (switch_is_up(rc_data[TEMP].rc.switch_right))// 右侧开关状态[上],无力
    {
        shoot_cmd_send.shoot_mode = SHOOT_OFF;
        shoot_cmd_send.load_mode=LOAD_STOP;
    }
    else if(switch_is_mid(rc_data[TEMP].rc.switch_right)) // 右侧开关状态[中],单发
    {
        shoot_cmd_send.shoot_mode = SHOOT_ON;
        shoot_cmd_send.load_mode =LOAD_1_BULLET;
    }
    else if(switch_is_down(rc_data[TEMP].rc.switch_right))// 右侧开关状态[下]，连发
    {
        shoot_cmd_send.shoot_mode = SHOOT_ON;
